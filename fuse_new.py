...
def fusion(radar_trace, location, loc_weight_param=10):
    ...
    C = dx.transpose().dot(dx) /(num_train-1)
    ...
    for i,point in enumerate(result):
        ...
        ret[i][7] = abs(delta_z / U[:,0][2])
        ret[i][0] = 0

    # Caculate timestamp.
    start_time = datetime.strptime(radar_trace[0][0].split('.')[0],'%Y-%m-%d %H:%M:%S')
    end_time = datetime.strptime(radar_trace[-2][0].split('.')[0],'%Y-%m-%d %H:%M:%S')
    delta_time = -(start_time-end_time)/num_train
    for i,res in enumerate(ret) :
        ob = start_time + i * delta_time
        res[0] = ob.strftime('%Y-%m-%d %H:%M:%S')
